...
bool CMT::initialize(const Mat im_gray, long topLeftx, long topLefty, long width, long height)
{
    // Region of interest - mkc
    ...
    // Cancel initialization if no keypoints are detected - mkc
    if (keypoints_fg.size() == 0 || keypoints_bg.size() == 0) {
        isInit = false;
        return isInit; 
    }
...
bool CMT::isInitialized() {
    return isInit;
}

bool CMT::processFrame(Mat im_gray, Mat &im_rgba) {
    ...
    isProcessed = consensus.findConsensus(points_fused, classes_fused, scale, rotation,
            voted_center, points_inlier, classes_inlier);

    //FILE_LOG(logDEBUG) << points_inlier.size() << " inlier points.";
    //FILE_LOG(logDEBUG) << "center " << voted_center;

    //Match keypoints locally
    vector<Point2f> points_matched_local;
    vector<int> classes_matched_local;
    matcher.matchLocal(keypoints, descriptors, voted_center, scale, rotation, points_matched_local, classes_matched_local);
...
//    classes_active = classes_fused;
    ...
    bb_rot = RotatedRect(voted_center,  size_initial * scale, rotation/CV_PI * 180);
    
        
    // Draw upright bounding box - mkc       
    Point2f rect_points[4]; 
    bb_rot.points( rect_points );
    for( int j = 0; j < 4; j++ ){
